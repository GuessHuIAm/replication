...
class ChatClient:
    def __init__(self, addr):
        ...
        # Flag to signal termination/exiting of the program
        ...
        # List detailing which servers are available
        self.server_available = [True] * self.NUM_REPLICAS
        for i in range(self.NUM_REPLICAS):
            # Send initial heartbeat to each of the server stubs, and if they are unresponsive, move to the next
            try:
                self.STUBS[i].Heartbeat(pb2.NoParam())
            except grpc._channel._InactiveRpcError:
                self.server_available[i] = False

        # Determine primary server
        self.determine_primary()
        # print(f'Replica {self.primary_index} chosen as the primary server.')
    ...
    def create_account(self, username, password):
        ...
        if not self.server_available[self.primary_index]:
            self.determine_primary()
    ...
    def listen_messages(self, username):
        ...
        # So we determine a new primary replica
            ...
            if self.exit_flag:
                return
            self.server_available[self.primary_index] = False
            self.determine_primary()
    ...
    def determine_primary(self):
        ...
        while self.primary_index < self.NUM_REPLICAS:
            if self.server_available[self.primary_index]:
    ...
    def heartbeat_servers(self):
        def heartbeat(index):
            try:
                for _ in self.STUBS[index].ContinuousHeartbeat(pb2.NoParam()):
                    if self.exit_flag:
                        break
            except grpc._channel._MultiThreadedRendezvous:
                self.server_available[index] = False

        self.threads = [Thread(target=heartbeat, args=(i,)) for i in range(self.NUM_REPLICAS)]
...
if __name__ == '__main__':
    ...
    th = Thread(target=client.listen_messages, args=(username,))
    th.start()
